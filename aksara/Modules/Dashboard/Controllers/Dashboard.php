...
class Dashboard extends \Aksara\Laboratory\Core
{
	public function __construct()
	{
		...
		if('count_upload' == service('request')->getPost('request'))
		{
	...
	private function _visitors()
	{
		...
		{
			if(file_exists(WRITEPATH . 'logs' . DIRECTORY_SEPARATOR . ($val ? date('Y-m-d', strtotime('-' . $val . ' day')) : date('Y-m-d')) . '.json'))
			{
				$get_log							= file_get_contents(WRITEPATH . 'logs' . DIRECTORY_SEPARATOR . ($val ? date('Y-m-d', strtotime('-' . $val . ' day')) : date('Y-m-d')) . '.json');
